...
def load():
    ...
    def loading(twodigit, subx, suby, loadinvent, align, alignxp):
        ...
        x = int(info[5 + align]) * subx
        y = int(info[6 + align]) * suby
        g = int(info[7 + align])
        if twodigit:
            gold = (info[7], info[8])
            g = int(''.join(gold))
...
while True:
    ...
    # LMAO Unfinished
        ...
        print("The creator is unfinished with the game so there is nothing here.")
